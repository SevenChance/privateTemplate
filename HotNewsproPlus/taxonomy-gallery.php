...
<?php endif; ?>
		...
		<div class="picTags">类别: <?php echo get_the_term_list($post->ID,  'gallery', '', ', ', ''); ?>
		    <?php echo get_the_term_list($post->ID,  'picture_tags', '标签: ', ' ', ''); ?></div>
		<div class="picLinkBox">
		    <span class="estorm_date">自<?php the_time('Y-m-d') ?></span>
		    <span class="estorm_views">被围观<?php the_views();?>+</span>
            <span class="estorm_comm"><?php comments_popup_link( '抢沙发', '坐板凳', '% 评论'); ?></span>
        </div>
        <div class="clear"></div>
        </div>
		<?php endwhile;?>
